...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Since we don't have a direct user creation mutation yet, redirect to login page
      // In a real application, you would create the user account first using a tRPC mutation
      router.push("/login");
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-md bg-[#0c0c24] p-8 rounded-xl shadow-xl border border-white/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full py-3 px-4 bg-gradient-to-r from-violet-600 to-cyan-400 text-white rounded-md font-medium hover:opacity-90 transition-opacity ${
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-[#0c0c24] text-gray-400">Or sign up with</span>
            </div>
          </div>
